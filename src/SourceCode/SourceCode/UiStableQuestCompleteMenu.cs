...
public class UiStableQuestCompleteMenu : KAUIMenu
{
	...
	public void PopulateItems(TimedMissionSlotData slotData)
	{
		...
		for (int i = 0; i < slotData.PetIDs.Count; i++)
		{
			...
			if (FreeStableQuestDragon.pInstance.pRewards != null)
			{
			...
			kAWidget2.FindChildItem("TxtDragonName").SetText(sanctuaryPetTypeInfo._NameText.GetLocalizedString());

...
public class UILabel : UIWidget
{
	...
	public NGUIText.Alignment pDefaultAlignment { get; private set; }
	...
	public string text
	{
		...
		set
		{
			...
			{
				if (!mIsEnglishTextCached)
				{
					mEnglishTxt = mText;
					mIsEnglishTextCached = true;
				}
				if (UtUtilities.IsLocaleRTL())
				{
					bool flag = UtStringUtil.IsRTL(value);
					if (pDefaultAlignment == NGUIText.Alignment.Left || (pDefaultAlignment == NGUIText.Alignment.Automatic && (base.pivot == Pivot.Left || base.pivot == Pivot.TopLeft || base.pivot == Pivot.BottomLeft)))
					{
						alignment = ((!flag) ? NGUIText.Alignment.Left : NGUIText.Alignment.Right);
					}
					if (flag)
					{
						string text = (mText = StringTable.ApplyRTLfix(value));
						MarkAsChanged();
						ProcessAndRequest();
						string[] array = processedText.Split('\n');
						int num = 0;
						_ = fontSize;
						while (array.Length != text.Split('\n').Length && num < 3)
						{
							fontSize = finalFontSize;
							string text2 = array.OrderByDescending((string s) => s.Length).First();
							int i = 0;
							int num2 = text2.Length;
							if (text.Length < text2.Length * 2)
							{
								num2 = text.Length - text2.Length;
								i = 2;
							}
							int num3 = num2;
							for (; i < 3; i++)
							{
								text = StringTable.ApplyRTLfix(value, num3);
								text2 = (from s in text.Split('\n')
								...
								num3 -= text2.Length - num3;
							}
							mText = text;
							MarkAsChanged();
							ProcessAndRequest();
							array = processedText.Split('\n');
							num++;
						}
					}
					else
	...
	public bool ignoreFontChange => mIgnoreFontChange;

	public string englishTxt => mEnglishTxt;
	...
	public void ProcessText(bool legacyMode = false, bool full = true)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			while (num3 > 0)
			{
				...
				bool flag3 = NGUIText.WrapText(printedText, out mProcessedText, keepCharCount: false, wrapLineColors: false, mOverflow == Overflow.ClampContent && mOverflowEllipsis);

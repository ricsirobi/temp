...
public class UICamera : KAMonoBase
{
	...
	public static GameObject selectedObject
	{
		...
		set
		{
			...
			if ((bool)mSelected)
			{
				UICamera uICamera2 = FindCameraForLayer(mSelected.layer);
				if (uICamera2 != null)
				{
					current = uICamera2;
				}
			...
			{
				UICamera uICamera3 = ((mSelected != null) ? FindCameraForLayer(mSelected.layer) : list[0]);
				if (uICamera3 != null)
				{
					current = uICamera3;
					currentCamera = uICamera3.cachedCamera;
				}
			}
			if ((bool)mSelected)
			{
	...
	{
		if ((current != null && current.NotifyEvent(go, funcName, obj)) || mNotifying > 10)
		{
			return;
		}
	...
	protected virtual bool NotifyEvent(GameObject go, string funcName, object obj)
	{
		return false;
	}
	...
	{
		if (Input.GetMouseButtonUp(0) || _IsMouseButtonUp)
		{
	...
	public virtual void ProcessMouse()
	{
		...
		if (flag || flag3 || mNextRaycast < RealTime.time)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				mMouse[j].current = currentTouch.current;
			}
		}
		...
		bool mouseButtonDown = GetMouseButtonDown();
		bool mouseButtonUp = GetMouseButtonUp();
		currentTouch = mMouse[0];
		currentTouchID = -1;
		currentKey = KeyCode.Mouse0;
		if (mouseButtonDown)

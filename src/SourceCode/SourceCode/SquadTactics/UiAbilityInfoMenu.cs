...
public class UiAbilityInfoMenu : KAUIMenu
{
	public void ShowEffectDetails(Character character, Ability ability)
	{
		...
		foreach (Effect effect in effects)
		{
			...
			if (kAWidget2 != null)
			{
				if (effect is Tick)
				{
					...
					string text = effect._InfoText.GetLocalizedString().Replace("{stat}", " [c]" + UtUtilities.GetKAUIColorString(statInfoByName._Color) + character.pCharacterData._Stats.GetStatValue(tick._InfluencingStat) + " [/c][ffffff]");
					text = text.Replace("{base}", tick._BaseAmount.ToString());
					text = text.Replace("{mult}", tick._Multiplier.ToString());
					text = text.Replace("{amount}", (character.pCharacterData._Stats.GetStatValue(tick._InfluencingStat) * tick._Multiplier + (float)tick._BaseAmount).ToString());
